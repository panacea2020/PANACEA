<template>
  ...
  >
    ...
    <div class="middle_box search">
      <p class="maintopic" style="margin-bottom: 0px; line-height: 90%;">
        PANACEA
      </p>
      <p
        class="topic"
        style="font-size: 18px; margin-left: 250px; margin-top: 0px; margin-bottom: 50px;"
      >
      ...
      <form name="form1" action="result_article.html" method="get">
        <input
          v-model="claim"
          class="fillmessage"
          type="text"
          style="width: 80%;"
          name="query"
          placeholder="Enter a fact to be checked"
          id="input_query"
        />
        <button class="jumpbutton" @click="updateClaim(claim)">
          GET STARTED
        </button>
      </form>
    </div>
...
export default {
  ...
  methods: {
    updateClaim(claimNew) {
      ...
      this.$router.push({path:"/list/article",query:{input:this.claim}});
    },
    ...
    readFile() {
      ...
      overallScore = this.getOverallStance(overallScore);
